...
const yupSchema = yup.object({
  ...
  confirmPassword: yup
    .string().required("không được để trống")
...
export default function Register() {
  ...
  const handleOnSubmit = async (data: initialFormState) => {
    try {
      const result = await http.post("register", {data, role:"user"});
      if (result) {
  ...
  return (
    <div className="w-full flex h-[100vh] justify-center bg-[#F8F8F8] items-center">
      <div className=" max-w-md p-8 space-y-3 rounded-xl bg-gray-900 text-gray-100">
        ...
        >
          ...
          <div className="space-y-1 text-sm">
            <label htmlFor="password1" className="block text-gray-400">
              Confirm Password
            </label>
            <input
              type="password"
